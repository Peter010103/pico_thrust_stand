...
uint16_t DShot::dshot_code_telemtry_to_command(const uint16_t code,
                                               const uint16_t t) {
  return code << 1 | t;
}
...
void DShot::packet_to_pwm(const uint16_t &packet, uint32_t pwm_buffer[],
                          const uint16_t &DShot_low, const uint16_t &DShot_high,
  ...
  // This is another interesting way to do it (assuming packet is passed as a
  // rvalue) Source:
  // https://stackoverflow.com/questions/2249731/how-do-i-get-bit-by-bit-data-from-an-integer-value-in-c#:~:text=can%20be%20further-,changed,-to
...
*/
void DShot::command_to_pwm_buffer(const uint16_t &code,
                                  const uint16_t &telemetry,
                                  uint32_t pwm_buffer[],
